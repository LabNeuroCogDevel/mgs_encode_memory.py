...
class showCal:
    def __init__(self, w=None):
        ...
        self.outerC = visual.Circle(self.w,units='pix',radius=self.outerRadius,lineWidth=0)
        self.innerC = visual.Circle(self.w,units='pix',radius=innerRadius,lineWidth=0)
        self.txt    = visual.TextStim(self.w, name='num', color='black',units='pix')
    ...
    def show(self, rednum=9):
        ...
        for xi in range(self.ncol):
            ...
            for yi in range(self.nrow):
                yi = yi+1
                cy = self.offset + self.spacingy*(yi-1) - self.w.size[1]/2
                cy = -1 * cy
